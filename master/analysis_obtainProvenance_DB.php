...
*/
  ...
  $targetCustomAnalysisIdList =$firstCustomAnalysisId;
	$query="select distinct Custom_Analysis_Id,Thread_Step_Number,trim(DB_script) as  DB_script,Project_Owner,Project,Thread_Name,Analysis_Name,Sql_delimiter
	         from MB_CUSTOM_ANALYSIS where Custom_Analysis_Id in ($targetCustomAnalysisIdList)
	         and DB_Script is not null and trim(DB_Script)!=''
    ...
    <body style="text-align: center">
       <span style="text-align: center">
       	DB tables used for data extraction (provenance) for:<br>
       	  ...
       	  <br><img height="24" style="vertical-align:bottom" src="images/analysisStep_small_medium.png"><?php print $allStepNmb[0] ?>:
       	  	&nbsp;&nbsp;<?php print $allAnalysisName[0] ?>
       	  <!--
        ...
        if(! array_key_exists($subQueryNmb,$distinctSubQuery) ) { 
      	...
      	if( $tblType =='perm') {
      	  ...
      	  print 'shapes['.$s.'].attr({stroke:"#00f", title:"'.$crtTableName.'"});'."\n";
      		...
      		if($crtTableName=='final'){print 'shapes['.$s.'].attr({stroke:"#0f0", title:"'.$crtTableName.'"});'."\n";}
      	  else                      {print 'shapes['.$s.'].attr({stroke:"#f00", title:"'.$crtTableName.'"});'."\n";}
      	}     
