...
*/
  ...
  if(checkWriteExecuteAccess()){
  	$gitCommandDb=getCfgVal('//svn_server')."commit_to_git.pl?what=Db_script&amp;Custom_Analysis_Id=".$mainId ; # an internal user
  	$gitCommandR =getCfgVal('//svn_server')."commit_to_git.pl?what=R_script&amp;Custom_Analysis_Id=".$mainId ; # an internal user
  } else {         
  	$gitCommandDb ="analysis_openEditor_ConsoleBlocked.html"; # an external user
  	$gitCommandR  ="analysis_openEditor_ConsoleBlocked.html";
  }
...
?>
	  ...
	  <html>
		<head>
			<link rel="icon" href="images/task.ico" />		
        ...
        <style type="text/css">
          ...
          td, img, textarea {vertical-align:middle}

        	table.info {color:#BBBBBB; font-size:80%}

          input.clickMe {border-radius:6px; width:30px; padding:1px; cursor:pointer }
              ...
              <table class="info">
								<tr>
									<td style="text-align:left; vertical-align:middle" colspan="4">
										<img alt="" src="images/owner_minus_small.png" height="24" title="Project owner"><textarea name="Project_Owner" id="col_1" rows="1" cols="30"><?php print $projectOwner; ?></textarea>
                ...
                <tr>
                	...
                	<td style="text-align:left" colspan="3">
									  <img alt="" src="images/project_minus_small.png" height="24" title="Project Name"><textarea name="Project" id="col_2" rows="1" cols="40"><?php print $project; ?></textarea>
                ...
                <tr>
                	...
                	<td style="text-align:left" colspan="2">
						 			  <img alt="" src="images/thread_minus_small.png" height="24" title="Thread Name"><textarea name="Thread_Name" id="col_3" rows="1" cols="60"><?php print $threadName; ?></textarea>
                ...
                <tr>
                	<td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td>
                	<td style="width:100%">
                		<table style="width:100%; font-size:100%;" class="info">
                		<tr>
                			<td>	
									      &nbsp;&nbsp;&nbsp;&nbsp;<img alt="" src="<?php print $stepImageHTML; ?>" height="16" title="Step Number"><input name="Thread_Step_Number" id="col_4" size="5" value="<?php print $Thread_Step_Number; ?>">
									    ...
									    <td align="left" style="vertical-align:middle">
									    	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;									    		
					              <img alt="" style="cursor:pointer; vertical-align:middle;" src="images/attachment.png" width="22" title="Manage Attached files" onclick="rollOn('tip_H_controlPanel')">
					              manage attachments
                      </td>
                      <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                    </tr>
							...
							<table class="info">
								<tr>
									...
									<td style="text-align:left">
										<textarea name="Analysis_Author" id="col_5" rows="1" cols="30"><?php print $analysisAuthor ?></textarea><img alt="" id="crtAuthorPictureSrc"  src="images/Icons/<?php print $authorPicturePath ?>" width="20" title="<?php print $analysisAuthor ?>">
								...
								<tr>
									...
									<td style="text-align:left">
										<textarea name="Analysis_Name" id="col_6" rows="1" cols="80"><?php print $analysisName ?></textarea>
								...
								<tr>
									...
									<td class="main_data_info">
										...
										<br>(wiki or markdown for knitr)
										<input class="clickMe" type="button" title="open wiki or markdown for knitr editor in a new tab" onclick="openEditor('col_15','Description',<?php print $mainId ?>,'<?php print "w$mainId" ?>')" style="background-color:#FDEADA;" value=". . . ">:
									</td>
									<td>
						    	...
						    	<td class="main_data_info">
						    		<span style="cursor:help" title="SQL script to be run on the DB server; result can be attached or re-generated with a call from the BioInfo/Stats script">data pre processing
						    			...
						    			<img alt=""  style="cursor:pointer" 
						    			  onclick="javascript:{if(popup) popup.close();popup=window.open('<?php print $gitCommandDb ?>','View/commit to versioning','height=420,width=600, left=150, top=150, directories=0, location=0, menubar=0, status=0, titlebar=0, scrollbars=1');}" 
						    			  title="view/update svn/git versioning with this SQL script that is stored in DB" src="images/github-fork.png" height="16">
						    		&nbsp;
						    		<input class="clickMe" type="button" title="open/run script in a separate colored syntax editor" onclick="openEditor('col_10','Db_script',<?php print $mainId ?>,'<?php print "s$mainId" ?>')" style="background-color:#E9EFDC;" value=". . .">
						    	...
						    	<td class="main_data_info">
						    		...
						    		<!-- popup=window.open('about:blank','Manage attachments','height=20,width=600, left=150, top=150, directories=0, location=0, menubar=0, status=0, titlebar=0'); document.attachFile_form.submit(); return true} -->  
						    			<img alt=""  style="cursor:pointer" 
						    			  onclick="javascript:{if(popup) popup.close();popup=window.open('<?php print $gitCommandR ?>','View/commit to versioning','height=420,width=600, left=150, top=150, directories=0, location=0, menubar=0, status=0, titlebar=0,scrollbars=1');}" 
						    			  title="view/update svn/git versioning with this script that is stored in DB" src="images/github-fork.png" height="16">
						    		&nbsp;<input class="clickMe" type="button" title="open/run script in a separate colored syntax editor" onclick="openEditor('col_11','R_script',<?php print $mainId ?>,'<?php print ".$mainId" ?>')" style="background-color:#DBEEF4;" value=". . .">
						    		<br><!--input type="button" value="get data provenance"-->
						    	</td>
						    	<td>
...
?>
    ...
    <form name="attachFile_form" method="post" action="analysis_editItem_manageAttachments.pl" target="Manage attachments" style="display:inline" enctype="multipart/form-data">
        ...
        <hr>
        <b>New attachment</b>:
        <br>Enter a short description<input name="short_description" size="20" title="short description of the content">
				<br><input type="file" style="background-color:#DDDDDD;" name="local_file" size="20">
				 
        <input style="cursor:pointer" type="button" onclick="if(confirm('[Insert?] or [Update when existing same `short description`?]')){popup=window.open('about:blank','Manage attachments','height=20,width=600, left=150, top=150, directories=0, location=0, menubar=0, status=0, titlebar=0'); popup.focus(); document.attachFile_form.submit(); return true}" name="__Attach" value="Attach file" title="Attach a new file to this analysis after Browsing and selecting it.">				  
		  </div>
    </form>									  
		...
		<form name="formOpenEditor" action="analysis_openEditor.php" method="post" target="<?php print $mainId?>">
      ...
      var editorWindow= Array();
      
			function openEditor(objId,objName,primaryKey,myTarget){
				if(editorWindow[myTarget]){ editorWindow[myTarget].close()}
				editorWindow[myTarget]=window.open('',myTarget);
